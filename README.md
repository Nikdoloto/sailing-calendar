...
Пример проксирования, если приложение висит на том же домене в корне:
...
Если приложение размещается в подпапке, например `https://niknotes.online/sailing-calendar/`, важно отдавать `index.html`, `manifest.webmanifest`, `service-worker.js` и `icons/*` из этой же подпапки. Manifest использует относительные `start_url` и `scope`, поэтому при добавлении на главный экран iPhone должен открывать именно `/sailing-calendar/`, а не корень домена.

На iPhone Safari не поддерживает программный install prompt как Chrome на Android. Поэтому кнопка `Добавить` в приложении показывает инструкцию, а фактическая установка делается через `Поделиться -> Добавить на главный экран`.
